...
class DratekTransfer:
    ...
    async def _send_once(
        ...
        async with BleakClient(connection_target, timeout=20.0) as client:
            ...
            # Do not switch between ATT write commands and write requests inside
            # one image stream. Some BlueZ adapters never complete the first
            # confirmed request after a no-response burst, so the final block is
            # lost and the display cannot refresh. Streaming firmware can use one
            # uniformly paced command stream whenever the characteristic offers it.
            unconfirmed_stream = (
                streaming_mode
                and "write-without-response" in write_char.properties
            )
            require_gatt_response = (
                not unconfirmed_stream
                and (

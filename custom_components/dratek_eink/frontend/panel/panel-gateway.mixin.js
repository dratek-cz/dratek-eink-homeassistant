...
export const gatewayMixin = {
  ...
  _renderGatewayWorkspace() {
    ...
    if (this._gatewaySubtab === "create") {
      ...
      const headActions = `<div class="gateway-view-actions"><button id="refreshSerialPorts" class="secondary refresh-ports-btn" ${this._gatewayBusy ? "disabled" : ""}><ha-icon icon="mdi:usb-port"></ha-icon>Načíst porty</button></div>`;
      const intro = "Připojte ESP přes USB přímo k zařízení s Home Assistantem. Vyberte port, typ desky a Wi-Fi síť.";
  ...
  _renderFlashResult() {
    ...
    const message = running
      ...
      : this._flashResult.ok ? "ESP32 gateway byla úspěšně flashnuta a Wi-Fi konfigurace odeslána." : `${this._flashResult.firmware_flashed ? "Firmware je nahraný. Nastavení Wi-Fi selhalo" : "Flash selhal"}: ${this._flashResult.error || "neznámý problém"}`;

...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn("terminal_barrier_writes = (", source)
        ...
        self.assertIn('"paced burst with first/final GATT barriers"', source)
        self.assertIn('"vendor write-complete flow control"', source)
        self.assertIn("block_number == total_blocks - 1", section)
        self.assertIn("block_requires_response = require_gatt_response or (", section)
        self.assertIn("block_number in {next_block, total_blocks - 1}", section)
        ...
        self.assertIn("paced_stream_complete", section)

...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn("bluez_acquired_stream = (", source)
        self.assertIn("not bluez_acquired_stream", source)
        self.assertIn("not force_confirmed_writes", source)
        self.assertIn('and "write-without-response" in write_char.properties', source)
        self.assertIn('"BlueZ AcquireWrite flow-controlled stream"', source)
        ...
        self.assertIn("await self._write_bluez_acquired_block(", section)
        self.assertIn('delivery = "BlueZ flow-controlled"', section)
        ...
        self.assertIn('member="AcquireWrite"', source)
        self.assertIn("written = os.write(fd, data)", source)
        self.assertIn("os.close(fd)", source)
        self.assertIn('"org.bluez.Error.InProgress",', source)

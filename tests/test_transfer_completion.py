...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn("STREAM_CHECKPOINT_SDK_TYPES = {299, 315}", source)
        self.assertIn("STREAM_CHECKPOINT_BLOCKS = 16", source)
        ...
        self.assertIn("or checkpoint_response", section)
        self.assertIn("block_number == total_blocks - 1", section)
        self.assertIn('"Display acknowledged checkpoint"', section)
    ...
    def test_gateway_supports_streaming_and_legacy_vendor_protocols(self):
        ...
        self.assertIn("if (streamingMode && !blockWriteWithResponse)", source)

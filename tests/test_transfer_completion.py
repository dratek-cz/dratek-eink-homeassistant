...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn("FINAL_BLOCK_DRAIN_DELAY = 1.0", source)
        self.assertIn("FINAL_BLOCK_RESPONSE_TIMEOUT = 2", source)
        self.assertIn("int(sdk_type) in WRITE_ACK_SDK_TYPES", source)
        self.assertIn('"write-without-response" in write_char.properties', source)
        self.assertIn('"paced burst with final GATT barrier"', source)
        self.assertIn("block_number == total_blocks - 1", section)
        self.assertIn("require_response=block_requires_response", section)
        self.assertIn("await asyncio.sleep(FINAL_BLOCK_DRAIN_DELAY)", section)
        self.assertIn("if require_gatt_response or terminal_barrier_writes", section)
        self.assertIn("if not streaming_mode or block_number != total_blocks - 1", section)

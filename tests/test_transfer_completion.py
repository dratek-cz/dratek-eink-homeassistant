...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn("ACK_CHECKPOINT_INTERVAL = 8", source)
        self.assertIn("ACK_CHECKPOINT_DRAIN_DELAY = 0.8", source)
        ...
        self.assertIn("checkpointed_writes = (", source)
        self.assertIn("not force_confirmed_writes", source)
        self.assertIn('and "write" in write_char.properties', source)
        self.assertIn('and "write-without-response" in write_char.properties', source)
        self.assertIn('"checkpointed burst (GATT response every', source)
        self.assertIn('"vendor write-complete flow control"', source)
        self.assertIn("block_number == total_blocks - 1", section)
        self.assertIn("(block_number + 1) % ACK_CHECKPOINT_INTERVAL == 0", section)
        ...
        self.assertIn("expected_checkpoints", section)
        self.assertIn("checkpointed_stream_complete", section)
        self.assertIn("expected_checkpoints.issubset(confirmed_blocks)", section)
        self.assertIn("len(sent_blocks) == total_blocks - 1", section)
        ...
        self.assertIn("await asyncio.sleep(ACK_CHECKPOINT_DRAIN_DELAY)", section)
        self.assertIn("force_confirmed_writes=attempt > 1", source)
        self.assertIn("Reliable retry enabled", source)

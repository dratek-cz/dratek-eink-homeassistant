...
class TransferCompletionTests(unittest.TestCase):
    def test_local_transfer_selects_the_vendor_protocol_from_software_version(self):
        ...
        self.assertIn('write_without_response = "write-without-response" in write_char.properties', source)
        self.assertIn("unconfirmed_stream = streaming_mode and write_without_response", source)
        self.assertIn("not write_without_response", source)
        self.assertIn('"uniform paced write without response"', source)
        self.assertIn('"notification-paced write without response"', source)
        ...
        self.assertIn("pace_without_response=streaming_mode", section)
        self.assertIn("if not require_response and pace_without_response:", source)
    ...
    def test_local_transfer_accepts_models_without_optional_completion_confirmation(self):
        ...
        self.assertIn("UNCONFIRMED_WRITE_DRAIN_TIMEOUT = 2", source)
    ...
    def test_gateway_supports_streaming_and_legacy_vendor_protocols(self):
        ...
        self.assertIn("blockWriteWithResponse = !writeChar->canWriteNoResponse()", source)
        self.assertIn("unconfirmedStream = streamingMode && !blockWriteWithResponse", source)
        self.assertIn("if (unconfirmedStream) delay(40);", source)
